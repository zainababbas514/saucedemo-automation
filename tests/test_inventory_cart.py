...
class TestInventoryCart(BaseClass):
    ...
    def test_inventory_items(self, login_credentials):
        ...
        for item in items:
            ...
            assert item["item_image"], (
                f"Image src is empty for {item['item_name']}"
            )

            # Name
            assert item["item_name"], "Product name is empty."

            assert item["name_displayed"], (
    ...
    def test_add_items_to_cart(self, item_count, login_credentials):

        allure.dynamic.title(
            f"TC-002: Add "
            f"{item_count['add_to_cart_item_count']} item(s) to Cart"
        )
        ...
        add_item_to_cart_count = item_count[
            "add_to_cart_item_count"
        ]
    ...
    @allure.story("Cart Management")
    @allure.title(
        "TC-003: Verify Added Items Persist and Match on Cart Page"
    )
    @allure.severity(allure.severity_level.CRITICAL)
    @allure.description(
    ...
    def test_cart_shows_added_items(self, login_credentials):
    ...
    def test_remove_item_from_cart(self, login_credentials):
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @allure.description(
        "Verify that clicking a product opens its details page."
    )
    def test_item_click_open_product(self, login_credentials):

        self.logger.info(
            "Starting TC-005: Verify that clicking a product "
            "opens its details page."
        )

        inventory_page = self.login_to_inventory(login_credentials)

        self.logger.info(
            "Clicking on a random product to open the "
        ...
        self.logger.info(
            "Verifying that the correct product details "
            "are displayed."
        )
